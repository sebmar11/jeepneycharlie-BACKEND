...
// Stores temporarily so staff can view + download it from the dashboard, then it gets deleted on approve/reject
app.post('/orders/:orderNumber/screenshot', upload.single('screenshot'), async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      `UPDATE orders SET screenshot_path=$1, payment_status='pending_verification', status='awaiting_verification', rejection_reason=NULL
...
// Downloads screenshot to staff device, deletes from server, unlocks QR for customer
app.patch('/orders/:orderNumber/approve-payment', async (req, res) => {
  try {
    const existing = await db.query('SELECT * FROM orders WHERE order_number=$1', [req.params.orderNumber]);
    if (!existing.rows.length) return res.status(404).json({ error: 'Order not found.' });
    const order = existing.rows[0];

    // Build download URL if screenshot exists (frontend will trigger download)
    const downloadUrl = order.screenshot_path ? `/orders/${order.order_number}/download-screenshot` : null;

    // Update order status
    const result = await db.query(
      `UPDATE orders SET payment_status='approved', status='pending' WHERE order_number=$1 RETURNING *`,
      [req.params.orderNumber]
    );

    res.json({ ...result.rows[0], screenshot_url: order.screenshot_path || null, download_url: downloadUrl });
  } catch (err) {
...
app.patch('/orders/:orderNumber/reject-payment', async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(
      `UPDATE orders SET payment_status='rejected', screenshot_path=NULL, status='awaiting_verification', rejection_reason=$1 WHERE order_number=$2 RETURNING *`,
      [reason || 'Payment screenshot rejected', req.params.orderNumber]
    );
    if (!result.rows.length) return res.status(404).json({ error: 'Order not found.' });
    res.json(result.rows[0]);
  } catch (err) {
...
// DELETE /orders/:orderNumber/delete-screenshot - deletes screenshot from server after staff downloads it
app.delete('/orders/:orderNumber/delete-screenshot', async (req, res) => {
  try {
    const existing = await db.query('SELECT screenshot_path FROM orders WHERE order_number=$1', [req.params.orderNumber]);
    if (existing.rows[0]?.screenshot_path) {
      const filePath = path.join(UPLOADS_DIR, path.basename(existing.rows[0].screenshot_path));
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);
      await db.query('UPDATE orders SET screenshot_path=NULL WHERE order_number=$1', [req.params.orderNumber]);
    }
    res.json({ ok: true });
  } catch (err) {
